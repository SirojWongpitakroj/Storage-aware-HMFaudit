...
// main function
func (tree *ShardTree) merge(first, second peak) (domain.MerkleNode, peak) {
	merkleT := tree.Tree
...
func (tree *ShardTree) Root() [32]byte {
	...
	for i := len(peaks) - 2; i >= 0; i-- {
		h = domain.HashPair(&merkleT.Nodes[peaks[i].height][peaks[i].index].Hash, &h)
	}
	return h
}
